...
class SimpleParameter(Node):
    def __init__(self):
        ...
        self.declare_parameter("simple_string_param", "August")
    ...
    def paramChangeCallback(self, params):
        ...
        for param in params:
            if param.name == "simple_int_parameter" and param.type_ == Parameter.Type.INTEGER:
                self.get_logger().info("New value for simple_int_param: %d" % param.value)
                result.successful = True

            if param.name == "simple_string_param" and param.type_ == Parameter.Type.STRING:
                self.get_logger().info("New value for simple_string_param: %s" % param.value)
                result.successful = True
